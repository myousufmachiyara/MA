            ...
            <form method="GET" action="{{ route('reports.accounts') }}" class="row g-2 mb-3 no-print">
                ...
                <div class="col-md-4">
                    <select name="account_id" data-plugin-selecttwo class="form-control select2-js">
                        ...
                        @foreach ($chartOfAccounts->when($key === 'party_ledger', fn($q) => $q->whereIn('account_type', ['customer','vendor'])) as $coa)
            ...
            <div class="table-responsive" id="report-table-{{ $key }}">
                <table class="table table-bordered table-striped align-middle table-sm">
                    <thead class="table-dark">
                        @if (in_array($key, ['general_ledger', 'party_ledger']))
                            <tr>
                                <th>Date</th><th>Account</th><th>Voucher / Ref</th><th>Narration</th>
                                <th class="text-end">Debit</th><th class="text-end">Credit</th><th class="text-end">Balance</th>
                            </tr>
                        @elseif ($key === 'trial_balance')
                            <tr><th>Account</th><th>Type</th><th class="text-end">Debit</th><th class="text-end">Credit</th></tr>
                        ...
                        @elseif (in_array($key, ['cash_book', 'bank_book']))
                            <tr>
                                <th>Date</th><th>Debit Account</th><th>Credit Account</th><th>Voucher / Ref</th><th>Narration</th>
                                <th class="text-end">Debit</th><th class="text-end">Credit</th><th class="text-end">Balance</th>
                            </tr>
                        @elseif ($key === 'journal_book')
                            <tr>
                                <th>Date</th><th>Voucher</th><th>Debit Account</th><th>Credit Account</th>
                                <th>Narration</th><th class="text-end">Amount</th><th class="no-print text-center">Actions</th>
                            </tr>
                    ...
                    @forelse ($reports[$key] ?? [] as $row)
                        ...
                        @if (in_array($key, ['general_ledger', 'party_ledger']))
                            ...
                            <td>
                                @if ($row['reference_link'])
                                    <a href="{{ $row['reference_link'] }}" target="_blank" class="ref-link">{{ $row['reference_label'] }}</a>
                                @else
                                    <span class="text-muted fst-italic">{{ $row['reference_label'] }}</span>
                                @endif
                            </td>
                        ...
                        @elseif (in_array($key, ['cash_book', 'bank_book']))
                            ...
                            <td><a href="{{ $row['reference_link'] }}" target="_blank" class="ref-link">{{ $row['reference_label'] }}</a></td>
                            <td class="narration">{{ $row['narration'] }}</td>
                            <td class="text-end">{{ $row['dr'] }}</td>
                            <td class="text-end">{{ $row['cr'] }}</td>
                            <td class="text-end fw-bold">{{ $row['balance'] }}</td>
                        ...
                        @elseif ($key === 'journal_book')
                            <td>{{ $row['date'] }}</td>
                            <td><a href="{{ route('vouchers.print', ['type' => $row['voucher_type'], 'id' => $row['voucher_id']]) }}" target="_blank" class="ref-link">{{ $row['voucher_no'] }}</a></td>
                            <td>{{ $row['dr_account'] }}</td>
                            <td>{{ $row['cr_account'] }}</td>
                            <td class="narration">{{ $row['narration'] }}</td>
                            <td class="text-end fw-bold">{{ $row['amount'] }}</td>
                            <td class="text-center no-print">
                                <a href="{{ route('vouchers.print', ['type' => $row['voucher_type'], 'id' => $row['voucher_id']]) }}" target="_blank" class="text-success me-1" title="Print"><i class="fas fa-print"></i></a>
                                @can('vouchers.edit')
                                <a href="#updateModal" class="text-primary modal-with-form" onclick="loadVoucherIntoModal('{{ $row['voucher_type'] }}', {{ $row['voucher_id'] }})" title="Edit"><i class="fas fa-edit"></i></a>
                                @endcan
                            </td>
...
{{-- Journal Voucher edit modal — manual vouchers only, matches VoucherController's edit guard --}}
@can('vouchers.edit')
<div id="updateModal" class="modal-block modal-block-primary mfp-hide">
    <section class="card">
        <form method="POST" id="reportUpdateVoucherForm" enctype="multipart/form-data" onkeydown="return event.key != 'Enter';">
            @csrf
            @method('PUT')
            <header class="card-header"><h2 class="card-title">Edit Voucher</h2></header>
            <div class="card-body">
                <div class="row">
                    <div class="col-lg-6 mb-2">
                        <label>Voucher #</label>
                        <input type="text" class="form-control" id="rv_display_no" disabled>
                    </div>
                    <div class="col-lg-6 mb-2">
                        <label>Date</label>
                        <input type="date" class="form-control" name="voucher_date" id="rv_date" required>
                    </div>
                    <div class="col-lg-6 mb-2">
...
<script>
function loadVoucherIntoModal(type, id) {
    const form = document.getElementById('reportUpdateVoucherForm');
    form.action = '/vouchers/' + type + '/' + id;

    fetch('/vouchers/' + type + '/' + id)
        .then(res => res.json())
        .then(data => {
            document.getElementById('rv_display_no').value = data.voucher_no;
...
function exportReportPDF(reportKey, reportLabel) {
    ...
    const html = '<!DOCTYPE html><html><head><meta charset="utf-8"><title>' + reportLabel + '</title>'
        ...
        + 'table{width:100%;border-collapse:collapse}th{background:#1a1a2e;color:#fff;padding:5px 7px;text-align:left}'
        + 'td{padding:4px 7px;border-bottom:0.5px solid #ddd}tr:nth-child(even) td{background:#f9f9f9}'
        + 'tfoot td{background:#e9ecef;font-weight:bold}.text-end{text-align:right}.fw-bold{font-weight:bold}'
        + '.table-active td{background:#dde;font-weight:bold}.narration{color:#888;font-style:italic}</style></head><body>'
        + '<h2>' + reportLabel + '</h2><p>Period: ' + from + ' to ' + to + ' &nbsp;|&nbsp; Account: ' + accountName + '</p>'
